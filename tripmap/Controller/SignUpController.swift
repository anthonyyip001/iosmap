...
class SignUpController: UIViewController, GIDSignInUIDelegate {
    ...
    let dontHaveAccountButton: UIButton = {
        let button = UIButton(type: .system)
        let attributedTitle = NSMutableAttributedString(string: "Already have an account?  ", attributes: [NSAttributedStringKey.font: UIFont.systemFont(ofSize: 16), NSAttributedStringKey.foregroundColor: UIColor.white])
        attributedTitle.append(NSAttributedString(string: "Sign In", attributes: [NSAttributedStringKey.font: UIFont.boldSystemFont(ofSize: 16), NSAttributedStringKey.foregroundColor: UIColor.white]))
    ...
    func createUser(withEmail email: String, password: String, username: String) {
        ...
        Auth.auth().createUser(withEmail: email, password: password) { (result, error) in
            ...
            self.userRef = db.collection("users").document(uid)
            
            self.userRef.setData(values) { err in
                if let err = err {
